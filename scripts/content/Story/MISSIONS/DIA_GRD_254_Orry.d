...
func void DIA_Orry_OldCamp_Info()
{
	...
	if(Npc_GetTrueGuild(hero) == GIL_None)
	{
		...
		B_LogEntry(CH1_JoinOC,"Strážce Orry mi vysvětlil, že každý, kdo přijde do Kolonie začíná jako kopáč. Pokud chci něco lepšího, měl bych se přidat mezi Stíny.");
	};
};
...
func void DIA_Orry_Waffe_Info()
{
	...
	AI_Output(self,other,"DIA_Orry_Waffe_06_04");	//A ještě něco ...
	AI_Output(self,other,"DIA_Orry_Waffe_06_05");	//Pravděpodobně tam najdeš i jiné věci, jako třeba staré mince.
	AI_Output(self,other,"DIA_Orry_Waffe_06_06");	//Myslím, že Diego takové sbírá.
	AI_Output(self,other,"DIA_Orry_Waffe_06_07");	//Jestli ti k ničemu nebudou, tak o nich řekni Diegovi. Určitě ti z toho něco kápne.
};
...
{
	AI_Output(self,other,"DIA_Orry_Message_15_00");	//Jestli jdeš do Starého tábora, mohl by jsi mi udělat laskavost.
	AI_Output(self,other,"DIA_Orry_Message_15_01");	//Můžeš něco někomu vzkázat?
	AI_Output(self,other,"DIA_Orry_Message_15_02");	//Až narazíš na chlápka jménem Pacho. Řekni mu, že další kolo je na mně.
	AI_Output(self,other,"DIA_Orry_Message_15_03");	//On už bude vědět o co jde.
	ORRY_MESSAGE = LOG_RUNNING;
	Log_CreateTopic(CH1_MESSAGEPACHO,LOG_MISSION);
	B_LogEntry(CH1_MESSAGEPACHO,"Strážce Orry mě požádal o laskavost, chce abych doručil zprávu muži jménem Pacho. Pokud toho chlápka potkám, měl bych mu říct, že další kolo bude na Orryho. Ať už to znamená cokoli.");
