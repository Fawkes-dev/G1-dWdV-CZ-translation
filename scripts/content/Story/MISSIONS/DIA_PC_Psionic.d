...
func void PC_Psionic_GUIDEFIRST_Info()
{
	...
	Info_AddChoice(PC_Psionic_GUIDEFIRST,"do volné chatrče.",pc_psionic_guidefirst_hut);
};
...
{
	AI_Output(other,self,"PC_Psionic_GUIDEFIRST_HUT_Info_15_01");	//… k nějaké neobsazené chýši.
	AI_Output(self,other,"PC_Psionic_GUIDEFIRST_HERB_Info_05_02");	//Pojď za mnou!
	LesterGuide = 5;
	AI_StopProcessInfos(self);
...
{
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_10");	//Vylez po tomhle žebříku a dostaneš se k volné chatrči.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_12");	//Normálně necháváme spát v našich chatrčích pouze členy tábora nebo nově příchozí.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_13");	//Takže buď opatrný, aby jsi do naší chatrče nevlezl jako člen jiného tábora.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_14");	//Templáři dokáží reagovat dost nelibě, pokud porušíš naše pravidla.
	AI_Output(self,other,"PC_Psionic_HUT_Info_06_11");	//Rozhlédni se kolem, počkám tě tu. Ale ať ti to netrvá dlouho, jinak se vracím zpátky ke vchodu.
	LesterGuide = 0;
	Npc_ExchangeRoutine(self,"START");
	AI_StopProcessInfos(self);
	TA_BeginOverlay(self);
...
func void PC_Psionic_BROTHERHOOD_TODO_Info()
{
	...
	{
		AI_Output(self,other,"PC_Psionic_BROTHERHOOD_TODO_05_05");	//Promluv si s Baal Namibem. Aby jsi mohl vstoupit na Chrámovou horu, budeš jako jeden z našich noviců potřebovat povolení od Guru. Poučí stráže, aby tě nechali projít.
	};
...
instance DIA_LESTER_MYOWNHUT(C_Info)
{
	...
	permanent = 1;
	description = "Můžu se tu někde vyspat?";
};
...
{
	AI_Output(other,self,"DIA_Lester_Hut_12_10");	//Můžu se tu někde vyspat?
	AI_Output(self,other,"DIA_Lester_Hut_12_20");	//Ano, jedna z chatrčí kousek od středu tábora se nedávno uvolnila.
	AI_Output(self,other,"DIA_Lester_Hut_12_30");	//Pokud chceš, můžu tě tam vzít a ukázat ti kde to je.
};
