...
func void DIA_Guy_MyOwnHut_Info()
{
	...
	AI_StopPointAt(self);
	AI_Output(self,other,"DIA_Guy_MyOwnHut_03_02");	//Ale říkám ti předem, budeš se tu moci zdržovat jenom pokud se přidáš ke Starému táboru anebo dokud se nerozhodneš ke komu se přidáš.
	AI_Output(self,other,"DIA_Guy_MyOwnHut_03_03");	//Pokud se přidáš k sektářům anebo k banditům a najdou tě v té chatrči Stráže, tak tě odtamtud honem vykopnou.
};
...
func void dia_guy_preparefight_info()
{
	if(HANIS_BET == TRUE)
	{
		AI_Output(self,other,"DIA_Guy_PrepareFight_15_00");	//Pospěš si, už brzo to začne.
		ARENA_FIGHT = FALSE;
		AI_StopProcessInfos(self);
