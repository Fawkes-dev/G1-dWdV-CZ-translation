...
func void Stt_311_Fisk_MordragKO_FuckOff()
{
	...
	B_LogEntry(CH1_FiskNewDealer,"Obchodníka Fiska z tržiště opravdu rozladilo, že jsem vyhnal Mordraga z tábora. Od této chvíle s ním nejsem zrovna zadobře.");
...
instance INFO_FISK_GILBERT(C_Info)
{
	...
	permanent = 0;
	description = "Znamená pro tebe něco jméno Gilbert?";
};
...
{
	AI_Output(other,self,"Info_Fisk_Gilbert_07_01");	//Říka ti něco jméno Gilbert?
	AI_Output(self,other,"Info_Fisk_Gilbert_07_02");	//Hmm ... ano, ten chlap chodil kolem a obchodoval se mnou. Ale dlouho jsem ho neviděl, ani netuším, kde je.
	B_LogEntry(CH1_GILBERT,"Fisk si stále pamatuje na Gilberta, ale neví o žádných potížích mezi ním a Gomezovými lidmi.");
};
...
instance STT_311_FISK_HUNOSSWORD(C_Info)
{
	...
	permanent = 1;
	description = "Nevíš něco o Hunově ukradeném meči?";
};
...
{
	AI_Output(other,self,"Stt_311_Fisk_HunosSword_15_00");	//Nevíš něco o Hunově ukradeném meči?
	AI_Output(self,other,"Stt_311_Fisk_HunosSword_12_01");	//Hm … Nevím … Nějak si nemohu vzpomenout …
	AI_Output(other,self,"Stt_311_Fisk_HunosSword_12_02");	//Mohl bych nějak osvěžit tvoji paměť?
	AI_Output(self,other,"Stt_311_Fisk_HunosSword_12_03");	//To mi řekni ty.
	Info_ClearChoices(stt_311_fisk_hunossword);
	Info_AddChoice(stt_311_fisk_hunossword,"Možná později.",stt_311_fisk_hunossword_back);
	Info_AddChoice(stt_311_fisk_hunossword,"A co 50 kousků rudy?",stt_311_fisk_hunossword_fault);
	Info_AddChoice(stt_311_fisk_hunossword,"A co třeba 100 kusů rudy?",stt_311_fisk_hunossword_right);
};
...
{
	AI_Output(other,self,"Stt_311_Fisk_HunosSword_Fault_15_00");	//Třeba 50 nugetů?
	AI_Output(self,other,"Stt_311_Fisk_HunosSword_Fault_12_01");	//Ne. Nic o tom meči nevím.
};

func void stt_311_fisk_hunossword_right()
{
	AI_Output(other,self,"Stt_311_Fisk_HunosSword_TakeIt_15_00");	//A co když řeknu 100 nugetů rudy?
	if(Npc_HasItems(other,ItMiNugget) >= 100)
	{
		AI_Output(self,other,"Stt_311_Fisk_HunosSword_TakeIt_12_01");	//Ach ano. Teď si vzpomínám.
		AI_Output(self,other,"Stt_311_Fisk_HunosSword_TakeIt_11_02");	//Nemohu ti toho říci mnoho. Promluv si s Fingersem. 
		AI_Output(other,self,"Stt_311_Fisk_HunosSword_TakeIt_12_03");	//To je vše?
		AI_Output(self,other,"Stt_311_Fisk_HunosSword_TakeIt_12_04");	//To je vše.
		B_GiveInvItems(other,self,ItMiNugget,100);
		Info_ClearChoices(stt_311_fisk_hunossword);
		HUNO_SWORDSOLD = TRUE;
		B_LogEntry(CH1_HUNOLOSTSWORD,"Fisk mi řekl, abych si promluvil se stínem jménem Fingers.");
	}
	else
	{
		AI_Output(self,other,"Stt_311_Fisk_HunosSword_TakeIt_12_02");	//Obávám se že nemáš dost rudy. Vrať se, až ji seženeš.
	};
};
